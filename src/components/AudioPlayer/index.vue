...
// 转换 file:// URL 为 API 端点
function convertAudioUrl(url?: string): string | undefined {
  if (!url) return undefined
  if (url.startsWith('file://')) {
    // 提取文件路径
    const filePath = url.replace('file://', '').replace(/\\/g, '/')
    return `/api/tts/audio?path=${encodeURIComponent(filePath)}`
  }
  return url
}
...
watch(
  () => props.audioUrl,
  (url) => {
    const convertedUrl = convertAudioUrl(url)
    if (convertedUrl && player.value) {
      player.value.load(convertedUrl)
...
function togglePlay() {
  const url = convertAudioUrl(props.audioUrl)
...
async function handleDownload() {
  ...
  if (!url) return
  try {
    const name = props.filename || `audio_${Date.now()}.${guessExtFromUrl(url)}`
