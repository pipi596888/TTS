...
function convertAudioUrl(url: string): string {
...
async function handleDownload(row: Work) {
  ...
  try {
    await downloadAudio(convertAudioUrl(row.audioUrl), downloadName(row))
...
function playWork(w: WorkView) {
  ...
  playerUrl.value = convertAudioUrl(w.audioUrl)
  currentTime.value = 0
  duration.value = 0
  const el = audioEl.value
  if (!el) return
  el.currentTime = 0
  // Let canplay event handler trigger play() when ready
}
